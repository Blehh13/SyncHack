...
| **Documentation matches the code** | [`scripts/verify-docs.mjs`](scripts/verify-docs.mjs), output in [`evidence/07-docs-claim-check.txt`](evidence/07-docs-claim-check.txt) | `npm run verify:docs` — fetches each page's Markdown from the live site and compares 15 documented claims against the source that implements them. Currently **8/15 pass**: the 7 failures are the drift Track did not fix. |
...
- **Agent-surface inconsistency.** Our API reference pages appear in `/api/docs-index` (18 entries) and in `llms-full.txt`, but `llms.txt` does not list them individually; it only points at `/api`. An agent reading `llms.txt` alone will not find the endpoint pages.
